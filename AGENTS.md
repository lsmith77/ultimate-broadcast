...
- `docs/APPLIANCE.md` — **a concept, nothing built, and deliberately undecided**: a box that renders an overlay URL on hardware we control at the field, rather than inside a switcher's undocumented browser. Its thesis is that the goal is lowering what an *operator* has to know, not cheaper hardware, and that decides most of the open questions in it. Three rungs ordered by crew skill — appliance, graphics source feeding a switcher, OBS — of which only the first needs building. Whether it gets built is left open on purpose; §12's phase 3 is the experiment and §12a is the stopping rule. Two things it says not to build: an OS image, and a wifi mesh.
- `docs/REPLAY.md` — **design notes, nothing built**: the operational half of `docs/APPLIANCE.md` §6c — triggering and tagging clips, reconciling two event streams that arrive in either order, assembling a playlist to fit a stoppage. Mostly drawn from a workflow actually operated on a Director Mini. Two rules travel: instant air needs an undo rather than a confirmation, and **injury marks an interval, never a clip**.
...
- `docs/BRAND.md` — **built**: the mark, the per-surface tab icons, and the social card. The rule it exists to protect is that the icons differ in **shape** and not only in colour, and that the palette was simulated under three colour vision deficiencies rather than assumed. Read it before changing an icon or a colour.
- `docs/ANALYTICS.md` — **built**: counting visitors and demo runs by reading the web server's access log, adding nothing to the software. `tools/visitors.php`, `robots.txt`, and the reason a tracker is not an option here — the scoreboard and the stage are on air, and a page that reaches air phones nowhere. Read it before proposing analytics; the imprint's "no analytics" claim is the thing being protected.
...
- `docs/STANDALONE.md` — running the overlays without UltiOrganizer. **It runs**, including authoring an event: `shared/provider.js` is the one way to read Live!, `shared/auth.php` the one place anything asks whether a request may reach air, `app.php` a front controller for a host that has no host, and `shared/event.php` the one place that knows the payload shape an event is written in. Read it before adding anything that reaches for UltiOrganizer, and use `Overlays\Mode` rather than writing `/live/overlays/` into a page — that assumption was in every one of them and broke the moment this directory was served from a document root.

...
A concept. Nothing is built, and whether the main version gets built is undecided. A pre-configured box renders one of this project's overlay URLs on hardware we control, at the field, instead of inside a switcher's undocumented browser.

This summary states each conclusion and links to the section that argues it.

---

## Summary

**Goal.** Make it feasible for tournaments to stream more games, at higher quality. That can mean more fields at one event or more events covered at once. Professionals with professional kit already achieve this. It is not affordable for these events, so the task is to reach a known standard on a budget that cannot pay for it. [§1a](#1a-the-goal-and-the-two-constraints)

**Two constraints.** Money, counted per field rather than per project. People, because complexity caps how many fields can run at once as surely as price does. Both bind, and they trade against each other. An answer has to satisfy both. [§1a](#1a-the-goal-and-the-two-constraints)

**What this project adds is the data, not the graphics.** Commercial tools already draw good scoreboards, but they need an operator to retype a score the scorekeeper has already entered. These overlays read that score, so the two cannot disagree. They read it from UltiOrganizer where there is one, and from [`MATCHCONTROL.md`](MATCHCONTROL.md)'s own store where there is not. Nobody keeps score twice. [§1b](#1b-two-working-rigs) · [§11](#11-cost)

**Capabilities are independent dials, not a single ladder.** Camera work, camera count, graphics, commentary, replay and uplink each get set where a tournament's money and people run out. Each dial can be filled with commodity hardware or with something bought to fix one specific problem. The software stays the same across all of them, so a tournament can equip six fields modestly and one properly without anybody learning two systems. A fixed bundle cannot do this: you cannot buy a third of a Director Mini, so six fields cost six times €1,300 even if five never need replay. [§1d](#1d-a-grid-of-independent-dials-not-a-ladder)

**Producing replays saves bandwidth.** Without replays, viewers do their own slow motion, so the high frame rate has to stay in the stream permanently. Producing them instead lets the wire carry 30p, saving three to four megabits per field at every event. A one-time hardware cost buys a permanent operating saving. [§6d](#6d-replay-needs-the-frame-rate-at-capture-not-on-the-wire)

**Live is an assumption, not a requirement.** Bandwidth limits how many games can be live. It does not limit how many can be covered, because every box records continuously and a recording uploads off-peak for almost nothing. This is the cheapest way to reach the stated goal. [§10a](#10a-the-network-at-a-field) · [§7a](#7a-local-recording)

**Two shapes.** They are peers, not a plan and a fallback:

| | what it is | cost | needs |
|---|---|---|---|
| **Graphics source** [§3b](#3b-the-graphics-source-the-box-out-of-the-video-path) | box stays *out* of the video path, hands a switcher a keyed overlay over HDMI | ~€60 | a switcher |
| **All-in-one** | box goes *in* the video path — camera in, composite, stream, record | €180–270 + capture | nothing else |

Every hard constraint on the box comes from being in the video path, so the graphics source avoids them entirely rather than reducing them. The uplink cost in [§10a](#10a-the-network-at-a-field) applies to the rig either way. Most of this document covers the all-in-one, because that is where the open questions are.

### The two hardware options

They do different jobs. The Pi suits the graphics source; the N100 suits the all-in-one. The prices are close enough that the choice comes down to power draw and encoding headroom.

| | **Raspberry Pi 4** | **Intel N100** |
|---|---|---|
| **cost, built** | ~€120, plus capture if it sits in the video path | €110–140 complete, including PSU, SSD, case and cooling, plus capture |
| **power** | 5–7W. A full tournament day from one power bank | 15–20W, so 3–4 hours. Most take a 12V barrel jack, but models with USB-C PD input exist in the same price class and are the ones to buy if battery matters ([§10b](#10b-running-off-a-battery)) |
| **best at** | the graphics source and the hub. Neither needs an encoder, so the hard parts of this document do not apply | the all-in-one. QuickSync does roughly ten times the encoding this needs |
| **as an all-in-one** | works at 1080p25/30 in hardware, with no headroom | comfortable, with room for a second simultaneous encode |
| **risk it adds** | lowest in the document as a graphics source. As an all-in-one it is the riskiest option: WPE on VideoCore is untested ([§13](#13-what-would-have-to-be-true)) and capture bandwidth is tight | fewer unknowns. Plain mainline Debian, better-supported graphics, and a mini PC form factor that removes three of [§10](#10-where-the-director-mini-is-ahead)'s field-reliability problems |
| **what it opens later** | add a switcher and it becomes the graphics source; replay via a laptop already in the rig | on-box replay, plus the high-rate capture buffer that makes its slow motion smooth ([§6d](#6d-replay-needs-the-frame-rate-at-capture-not-on-the-wire)); OBS if that fork is taken; and booting from a USB SSD on a borrowed laptop ([§5h](#5h-bring-your-own-hardware)) |
| **what it cannot do** | two encodes at once, so no replay on the box | run all day on a battery without planning for it |

The choice does not have to be made now. Phases 0–2 use a Pi either way, and the board is a phase-3 purchase made after [§12a](#12a-what-phase-3-must-prove-and-when-to-stop) has measured it. Detail in [§5f](#5f-is-a-pi-still-the-answer) and [§5g](#5g-comparing-the-boards-as-a-graphics-source).

### Settled

- **Build the graphics source before the all-in-one.** It is cheaper and simpler, and its topology already runs in a real rig. [§3b](#3b-the-graphics-source-the-box-out-of-the-video-path)
- **Never ship an OS image.** Use an `apt` package on a stock OS. An image means owning an operating system's security updates. [§9](#9-maintenance-no-os-image)
- **OBS with its UI visible is the documented upgrade path** for crews with the skills, and a better one than buying a switcher. Whether OBS should also be the appliance's hidden engine on x86 is still open, and is one of phase 3's first questions. [§7b](#7b-obs-instead-of-a-custom-pipeline)
- **Configuration comes from the tournament's own install,** never from a service we run, so no new party holds anybody's credentials. [§8b](#8b-configure-centrally-not-at-the-field)
- **Automated camera operation is bought, not built.** [§4d](#4d-automated-cameras-and-rtmp-only-sources)
- **A capture device is always required.** No board here has an HDMI input, and there is no way around that. [§4](#4-getting-video-in-a-capture-device-is-always-needed)
- **No wifi mesh and no AI accelerator.** Both look attractive; §10a and §5e explain why neither works. [§10a](#10a-the-network-at-a-field) · [§5e](#5e-does-an-ai-hat-help)

The decisions underneath these — frame rate, compositing method, credential handling, control surfaces — are argued in the body rather than summarised here.

### Open, with a current preference

| | leaning | where |
|---|---|---|
| Getting video in | network transport beats HDMI on cabling, power and placement — but costs an encoder per camera unless the camera speaks it natively | [§4c](#4c-why-network-input-wins-on-the-field) |
| Replay | off the box: about €10 wherever a laptop is already in the rig, expensive or absent on a bare appliance | [§6b](#6b-replay-off-the-box) |
...
Whether the all-in-one gets built at all. Phase 3 in [§12](#12-what-to-do-first) is the experiment, and [§12a](#12a-what-phase-3-must-prove-and-when-to-stop) sets the stopping rule in advance. Phases 0–2 are worth doing whatever the answer, so read §§4–7 as a list of what phase 3 must measure rather than as a construction plan.
...
| **3** | bench the streaming pipeline; **[§12a](#12a-what-phase-3-must-prove-and-when-to-stop) decides here** | nothing |
...
Several sections only make sense against these assumptions.

- **This is a product other tournaments deploy, not a rig for one crew.** That is why [§9](#9-maintenance-no-os-image) rejects an OS image and why [§8c](#8c-driving-the-platform-from-the-schedule)'s OAuth problem is hard. The unit is a rig rather than a field, and most rigs cover one field: a federation replacing one travelling rig with three is as good a reason to build as one tournament equipping six fields ([§1a](#1a-the-goal-and-the-two-constraints)). A fourth audience, DIY users attached to no association, is served by what already exists rather than by the box.
- **Stream and archive matter equally. A venue screen does not.** That is what confines hardware planes to [§3b](#3b-the-graphics-source-the-box-out-of-the-video-path).
- **Two roles: somebody technical installs, volunteers operate.** The target is the tournament morning, not the install.
...
### 1a. The goal and the two constraints

The goal is to let tournaments stream more games, at higher quality. Not one showcase field but several, and not merely present but watchable.

The benchmark is not in doubt. Hiring professionals with professional kit achieves all of this today and always has. It is simply not affordable for the events this is for. Nothing here needs inventing; it needs reaching a known standard on a budget that cannot pay for it.
...
The two trade against each other. Cheap usually means fiddly: a €200 box with a capture device and a pipeline, against a €1,300 appliance that works out of the box. Simple usually costs, because the Director Mini is expensive for having solved the hard parts. Satisfying either constraint alone is easy, and neither alone puts a second field on air.

Most conclusions in this document follow from that pairing:

- **Cheaper and simpler, so ranked first.** §3b's graphics source. §9's `apt` package over an OS image. §1c's €10 numpad over a €150 Stream Deck.
- **Cheap but complex, so rejected or ranked last.** §7b's OBS is free but asks an operator to know what a scene is. §4d's cheapest cameras save money but cost a quality generation and add a wifi hop.
- **A genuine trade, so left undecided.** §5d's x86 costs more and removes a whole class of difficulty. §12a settles it by measurement rather than argument.

#### The people constraint applies to the tournament morning, not the install

There are two roles: somebody technical installs a box, and volunteers run it on the day. So the install may be an `apt` command and an enrolment token (§9, §8b), while the operating surface has to work for somebody who will not open a terminal and cannot read a log.

Two consequences follow. Every failure has to be visible in the Studio rather than in `journalctl`, and §10's missing confidence monitor becomes a requirement rather than a nice-to-have.

#### Four audiences, and the unit is a rig

The constraint above says per field, but the unit is really a rig, and a rig often covers a single field. That matters because demand comes in four shapes and only one is about field count.

**One rig, one field.** The commonest case: a club tournament streaming one game at a time, or a large one streaming only the final. Almost everything this project offers is scale-independent and applies here in full. Nobody keeps score twice, the browser stops being unknown, the overlay can run itself from scorekeeper actions, the recording is a publishable VOD, and replay is data-driven. This case is served today by phases 0–2 in [§12](#12-what-to-do-first) and by overlays that already exist.

**One tournament, several fields.** Here the per-field arithmetic dominates and [§11](#11-cost) applies as written: six fields with the budget for one is not solved, so what counts is the cost of the next field.
...
The cheapest form is neither the appliance nor OBS. Two action cameras recording to their own cards, processed and overlaid afterwards ([§4d](#4d-automated-cameras-and-rtmp-only-sources)), need no box, no network at the field, no mains and nobody operating anything. The cameras stamp their own footage, which removes [`POSTPRODUCTION.md`](POSTPRODUCTION.md)'s alignment problem. For somebody with no association, no budget and no crew, that is the whole product.

DIY users are self-selected and motivated, so they tolerate complexity a volunteer cannot. That puts them on the top rung — their own laptop, OBS and the overlay URLs — rather than on the appliance. They need documentation and public availability rather than a federation partnership, so phase 0 in [§12](#12-what-to-do-first) does not apply to them and phases 1–2 do.

They cannot have support. Unaffiliated users on unknown hardware are [§5h](#5h-bring-your-own-hardware)'s known-good-list problem, and promising support would create an obligation nobody can meet. State the expectation instead: it works, it is documented, and it is best-effort. In exchange, they are how the other three shapes find out this exists, and the people most likely to find its bugs.

This resolves an apparent contradiction with [§11](#11-cost), which says the economics only work at scale and that a single rig is better off buying a Director Mini. Both are statements about whether to *build*, not about who the result is for. Many rigs satisfy that scale as well as many fields do, so a federation wanting three travelling rigs is as good a reason to build as one tournament wanting six fields. Each of those rigs may cover one field.

Two things follow for travelling rigs; [§10d](#10d-circulating-a-rig-without-it-coming-home) covers the logistics. A travelling rig is packed and unpacked by different people at every event, so it must carry no event-specific state. That is what [§8b](#8b-configure-centrally-not-at-the-field)'s central provisioning gives: the rig learns which field it is on arrival rather than being reconfigured before shipping. And several rigs need several update paths, which [§9](#9-maintenance-no-os-image)'s `apt` model handles better than posting SD cards around.

One argument the cost framing misses: for a club tournament with €200 rather than €2,000, this is not a saving but the difference between streaming and not streaming at all. That is an access argument rather than an economic one. It applies only to the one-rig-one-field case, and it may be the strongest case this project has.
...
The ideal is one platform that scales up and down with minimal change. One codebase across both shapes, both boards and every rung; anything else means two sets of bugs, two things to keep in step, and features that exist on one side and not the other. That pressure is real: it is why [§3b](#3b-the-graphics-source-the-box-out-of-the-video-path) and the all-in-one deliberately share almost everything, and why [§7b](#7b-obs-instead-of-a-custom-pipeline)'s engine question matters at all.
...
- **The control-surface pattern**, confirmed independently — a Stream Deck drives the graphics rather than a web UI ([§1c](#1c-hardware-controls-and-when-a-numpad-is-enough)).
- **Replay as a separate appliance** — though HyperSlow records nothing itself, it drives Blackmagic HyperDecks, so the capability is €600–800 rather than the price of an app ([§6b](#6b-replay-off-the-box)). A HyperDeck fed from an ATEM output most likely records clean, which would mean rig A solved [§3c](#3c-the-frozen-scoreboard-over-a-replay)'s frozen scoreboard by architecture.
- **Optical HDMI is a real alternative to network transport** — uncompressed, no encode hop, real money ([§4c](#4c-why-network-input-wins-on-the-field)).

The fourth is the one stated nowhere else as sharply: H2R Graphics already does the graphics, commercially, today. So the overlay layer is not this project's differentiator — **the data is.** H2R needs an operator typing a score a scorekeeper already entered, with a second chance to get it wrong; these overlays read it and cannot disagree with it. **Nobody has to keep score twice.** That is the value proposition, and [§11](#11-cost) carries it because any budget comparison that ignores it is arguing about the wrong thing.
...
### 1c. Hardware controls, and when a numpad is enough
...
One caveat on latency, since it is easy to conflate with this. A keypress reaches air through the show store and its ~1s poll, so a numpad is fast to *use* and not instantaneous in *effect*. [§2](#2-why-this-matters-beyond-cost) notes a box can push instead of polling, which removes that second for discrete actions. **It does not change anything above** — push is still not a jog wheel or a fader, so continuous controls stay on hardware regardless.
...
### 1d. A grid of independent dials, not a ladder
...
|---|---|
| nobody spare to operate a camera | XbotGo, or Pix4Team + camcorder ([§4d](#4d-automated-cameras-and-rtmp-only-sources)) |
| long HDMI runs, trip hazards, no power at the tripod | a network encoder, PoE ([§4c](#4c-why-network-input-wins-on-the-field)) |
| graphics too slow to fire by hand | a €10 numpad first, a Stream Deck only if that is not enough ([§1c](#1c-hardware-controls-and-when-a-numpad-is-enough)) |
| encoder has no headroom | x86 instead of a Pi ([§5d](#5d-x86-instead-of-a-pi)) |
...
- **Commentary has a range limit.** Wireless mics into the camera are the cheap answer ([§3a](#3a-audio)) and stop working when the desk is far from the camera.
...
## 2. Why this matters beyond cost
...
Be precise about what it buys, because it is narrower than it sounds. It removes up to a second from *operator actions* — a card going on air, and in particular [§6c](#6c-replay-and-the-data-advantage)'s replay trigger, where instant air on a button press is the whole design and a one-second poll cannot deliver it. **It does nothing for score latency**, which is bounded by the API's own cache life rather than by how the client asks.

**And it can only ever be an enhancement.** The overlays must keep working with no box — hosted, standalone, on somebody's laptop — so polling stays the floor and push is what the client prefers when a relay answers. That is a second path to maintain, which [§1a](#1a-the-goal-and-the-two-constraints) ranks third among costs and is worth paying only where the latency actually matters. **On the evidence so far that is the replay trigger and very little else.**
...
|---|---|---|
| **Multi-camera cutting**, PiP, transitions | **reachable, and deliberately declined** — the cost is not the switching, it is everything switching drags in | below, and [§3b](#3b-the-graphics-source-the-box-out-of-the-video-path) |
| **Recording** as a hedge against the uplink | **recovered, and better** — the box's copy has the overlay burned in, so it is publishable as a VOD | [§7a](#7a-local-recording) |
| **Audio** — mic inputs, mixing, monitoring | **mostly recovered** — wireless mics into the camera arrive embedded, with no second clock to drift | [§3a](#3a-audio) |
| **Replay** | **recovered off the box** at about €10 wherever a laptop is already in the rig; rich, data-driven replay is a real prospect; absent on a bare appliance | [§6b](#6b-replay-off-the-box) · [§6c](#6c-replay-and-the-data-advantage) |
...
A delayed source produces the mirror image of this — the graphic arriving *before* the picture rather than after it, which [§4d](#4d-automated-cameras-and-rtmp-only-sources) covers. Both are the same fault: the overlay and the video disagreeing about what time it is.
...
### 4b. HDMI and network input are separate projects
...
### 4d. Automated cameras and RTMP-only sources
...
Its operational advantages are real and undersold. Nothing to level, nothing to pan, nothing mechanical to fail, and a pair of GoPros or a Reolink is far more weatherproof than a robot on a tripod. That is [§1a](#1a-the-goal-and-the-two-constraints)'s people constraint answered by having nothing to operate — and it also **confirms [§5e](#5e-does-an-ai-hat-help)'s conclusion from the other side**: cropping a high-resolution wide shot is exactly the auto-framing this document declined to build, sold as a product.
...
And it makes [§10c](#10c-a-monitor-on-the-box)'s monitor mandatory rather than merely useful — for two reasons, of which the second is the sharp one.
...
Two things in that are worth pulling out. The fixed installation is **PoE-powered over a single Ethernet run**, which is exactly the arrangement [§4c](#4c-why-network-input-wins-on-the-field) argues for on cabling and placement grounds — arrived at independently. And the portable rig runs its cameras **off a power bank**, so at the camera end this tier is battery-friendly in a way the processing end is not.
...
**Which inverts where this tier sits.** The cameras are the cheap part; the computer is not. Against a €110–140 N100 the processing machine is a different order of expense, so **this is the most expensive tier here, not the middle one** — and per [§1a](#1a-the-goal-and-the-two-constraints) that cost lands per field, where it hurts most. Power depends on which route: **an RTX desktop ends [§10b](#10b-running-off-a-battery)'s battery story outright**, while an Apple Silicon Mac mini is a different proposition — a few hundred euro, low draw, silent, and not obviously off a large power station. **That is a materially better option than the GPU tower this section first assumed**, and [§1b](#1b-two-working-rigs)'s rig A already has a Mac in it, so it is not alien to this world.

But that points at the deployment it actually suits, which is not a travelling rig at all. Fixed cameras, a machine that stays put, mains power, nothing to pack or level or aim — **this is a club's permanent installation at its own ground**, streaming every home game, rather than a rig that visits tournaments. Read that way its costs stop being objections: a permanent site has mains, usually has wired internet, and has no packing list, which removes most of [§10](#10-where-the-director-mini-is-ahead)'s field problems at a stroke, and the hardware is bought once rather than per event.

#### On this tier the overlay problem is already solved, and so is the audio

**Once's own answer to graphics is OBS.** Their documentation points users at it for scoreboards and sponsor banners, compatible with both the camera and Autocam. So there is no competing product here and no overlap to worry about — **OBS is already in the chain**, and these overlays go into it as a browser source. That is [§1a](#1a-the-goal-and-the-two-constraints)'s top rung, reached with nothing built.
...
It also tidily confirms the verdict above rather than complicating it. This is a tier for streaming without replay, and 20 fps is one more reason why — alongside its being outside both regional families in [§6e](#6e-ntsc-regions).
...
It fits [§1e](#1e-other-sports)'s conclusion exactly — partner on the sport-agnostic half while the overlays stay Ultimate's — but **it is speculative rather than promising, and it is recorded here as the right shape rather than as an option.** Worth one question in the phase-0 conversation; not worth planning around.
...
Which costs less than it sounds, because the tier's audience is the one least likely to want replay anyway. A club's permanent installation streaming home games is a deployment with one operator or none, and [§6a](#6a-replay-on-the-box-later) already found that no crew size has anybody spare for it. **The tier and replay are effectively an either/or, and for the deployment this tier is actually for, that is not a loss.**

It also explains why this tier needs so little from the appliance: strip out replay and what remains is a camera, OBS and an overlay URL — [§1a](#1a-the-goal-and-the-two-constraints)'s top rung again.
...
### 5g. Comparing the boards as a graphics source
...
### 6c. Replay and the data advantage
...
Once replay is in scope, [§6](#6-frame-rate-50-fps-is-not-needed)'s reasoning needs revisiting — it ruled out 50p partly because nobody could play slow motion, and [§6b](#6b-replay-off-the-box) puts replay at about €10. The output conclusion survives; the requirement moves.
...
**Why would anyone stream 50/60 at all?** [§6](#6-frame-rate-50-fps-is-not-needed) gives two reasons: motion rendering on pans, and slow motion. And its consolation for having no replay was that *viewers can scrub a platform's DVR themselves* — but a viewer doing their own slow motion needs the high frame rate to be in the stream, because they are slowing down whatever we sent them. Not producing replays pushes the frame rate onto the wire, permanently.

Produce them instead and the slow motion happens **before** transmission, out of a buffer that never leaves the box. The stream then only has to carry the live picture, and the remaining case for 50p is motion rendering alone — which [§6](#6-frame-rate-50-fps-is-not-needed) already judged modest.
...
- **Frame rate belongs in the per-installation configuration** ([§8b](#8b-configure-centrally-not-at-the-field)), not in a constant. It is tournament-wide, known before anyone arrives, and exactly the sort of thing that section exists to deliver.
...
### 7b. OBS instead of a custom pipeline
...
**What it brings is substantial.** [§6c](#6c-replay-and-the-data-advantage) made replay the headline differentiator — and OBS has a replay buffer, an audio mixer with a limiter and meters ([§3a](#3a-audio)'s entire software list), encoder plumbing, reconnection and recording, all maintained by other people and packaged in Debian so [§9](#9-maintenance-no-os-image)'s model holds unchanged. Every one of those is something the pipeline route has to write.

There is also a structural argument that nothing else in this document offers. If the appliance runs OBS internally, then the appliance and [§1a](#1a-the-goal-and-the-two-constraints)'s skilled-crew rung are the same engine at two levels of exposure, rather than two codebases with two sets of bugs. That is *one product to build* made literal, and it is the cleanest version of the ladder yet.

One objection survives all of this, and it is the reason the question is not simply closed in OBS's favour. [§7c](#7c-running-obs-headless): a GUI application's error handling is a dialog box, and a modal on a screenless machine is a broadcast that has silently stopped — with obs-websocket's crash history, including a reported startup crash when a scene collection contains a browser source, which is every scene we would build. A pipeline under systemd fails by exiting, which a supervisor sees.

So the fork is open, and narrower than "pipeline or OBS" — but it is not a contest between equals. [§1a](#1a-the-goal-and-the-two-constraints) ranks operational simplicity above build effort, so OBS has to clear a bar rather than merely compete on points. Halving the work is worth a lot, and worth nothing if the box can stop silently at a field.
...
**None of this touches the Pi**, which settles cleanly the other way: OBS has no good hardware-encode path there, and [§3b](#3b-the-graphics-source-the-box-out-of-the-video-path)'s graphics source needs no engine at all. The engine question only exists on x86.

And it does not change where OBS sits for skilled crews. For a rig that already has a laptop, OBS with its UI *visible* remains the better scale-up than buying a switcher — it costs nothing, and it has the replay buffer and the control surface [§6a](#6a-replay-on-the-box-later) says are the blockers. That crew will run it on their own laptop rather than ours, which is why that rung needs almost nothing built: the overlay URLs, which exist, and phase 1's hub.

### 7c. Running OBS headless
...
### 8a. The stream key, the first real secret
...
And a travelling rig makes this a hard requirement rather than a preference ([§1a](#1a-the-goal-and-the-two-constraints)): a box passing through many hands must hold no secret at rest, so keys are fetched per event and never written to the disk that is moving.
...
### 8b. Configure centrally, not at the field
...
## 10. Where the Director Mini is ahead
...
- **Cellular per field**, which is what both rigs in [§1b](#1b-two-working-rigs) do — an LTE day pass, or a 5G router. Each island is genuinely independent, so one field's uplink failing costs only that field. The cost is a router per field plus data that recurs every event, and at a crowded venue it competes with several hundred spectators for the same cells.
...
And this is a recurring per-field cost, which [§1a](#1a-the-goal-and-the-two-constraints) says is the number that counts. Boxes are bought once; connectivity is paid for at every event, for every field, forever. Over a few seasons it plausibly exceeds the hardware — so "how does each field reach the internet" deserves to be asked before "which board", and it currently is not.
...
**Not every game has to be live.** [§7a](#7a-local-recording)'s recording already runs continuously on every box, already has the overlay burned in, and is already publishable with no post-production. So a field that cannot be live can still be **covered** — recorded now, uploaded when the field goes quiet between games or overnight, on capacity that is otherwise idle.

That reframes the constraint usefully. Bandwidth caps how many games can be **live**; it does not cap how many can be **covered**, because a recording costs roughly nothing to upload off-peak. And [§1a](#1a-the-goal-and-the-two-constraints)'s goal is more games — live was never the requirement, it was an assumption. For a pool game at nine in the morning the audience is mostly retrospective anyway.
...
- **Free today** — a per-field choice of *live* or *record and upload later*, plus a fixed bitrate per field, set in the central configuration [§8b](#8b-configure-centrally-not-at-the-field) already provides. A tournament gives the showcase field the bandwidth and records the rest. No coordination, no arbiter, nothing new.
...
That is a statement about the decision to build, not about who the result is for, and [§1a](#1a-the-goal-and-the-two-constraints) sorts the two. The scale in question is **rigs**, not fields: a federation replacing one travelling rig with three is exactly as good a justification as one tournament equipping six fields, and every one of those rigs may cover a single field. Most of the value here is scale-independent and arrives in full at one field — the build cost is the only part that needs numbers behind it, which is precisely the audience [`STANDALONE.md`](STANDALONE.md) is written for.
...
### 12a. What phase 3 must prove, and when to stop
...
Criterion 1 is engine-specific, and [§7b](#7b-obs-instead-of-a-custom-pipeline) reopened which engine that is. On the pipeline route it is `wpevideosrc` transparency. On the OBS route the equivalent gate is different and harder: **can OBS be made to fail detectably and hold state deterministically when driven headless** — a watchdog over obs-websocket plus a supervised restart, verified by breaking it on purpose. **Whichever engine is on trial, criteria 2 to 5 apply unchanged.** Settle the engine first, because it decides what criterion 1 even means.

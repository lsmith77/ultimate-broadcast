...
`--show` stops after deciding: it prints the source directory **and its mode**, plus the `version.json` that would be sent. It contacts nothing and needs no `deploy.env`.

**The mode is printed because it took the site down once.** `rsync --archive` copies the *source directory's* own permissions to the destination, and a git worktree made by `mktemp -d` is `0700` — so the first deploy from a tag set the live document root to `0700`, the web server (which is not the account that owns the files) could no longer traverse it, and **every URL answered 404 while every file sat there correct and complete**. Deploying from the working copy never showed it, because a checkout is `0755`. The worktree is now chmodded before use and `tests/deploy.mjs` asserts the mode.

The same deploy published a `.git` file naming a path on the deploying machine: a worktree's `.git` is a *file*, and the exclude was written `/.git/`, which matches only a directory.
...
**Tell scorekeepers to add `/k/<game>` to their home screen**, not the long URL. The Studio's games table has that link per game, under **Match control ↗**, and each game needs a scorekeeping code nominated in the same panel — entered on the phone while it still has signal, since a code cannot be checked at a pitch with no network. That short path is the only one the service worker covers: a worker's scope is a path, and widening it to reach the long form would put one in front of the scoreboard and the stage. Opening each game once while there is signal fills the offline cache. `/k/` on its own lists the games that phone is carrying. [`OFFLINE.md`](OFFLINE.md).

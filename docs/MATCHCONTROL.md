...
**Who else the spotter is depends on who supplied them, and the two models are not interchangeable.** A spotter the TOURNAMENT supplies most likely replaces the scorekeeper rather than joining them: they are already at the pitch, watching every point, and recording every goal as part of a much finer stream — the score is a strict subset of what they capture. That changes the staffing arithmetic from "one more person per pitch" to "the same person, recording more", which is a far easier thing to roster. It also removes the reconciliation problem, because one person cannot disagree with themselves about the score.

A spotter a TEAM supplies must not be the scorekeeper, and this is not a preference. A coach is partisan about exactly the calls the capture records — whose turnover it was, whether that was a drop or a throwaway — and §7's rule stands: the scoresheet is the tournament's record, not a broadcast artefact. A coach's capture is *input* to a broadcast and to their own analysis; the score stays with somebody independent. So the coach-supplied model needs a scorekeeper alongside it, and the tournament-supplied model can absorb one, even though both produce the same shape of data.

**The tournament-supplied spotter is the first target, and the only one being designed for now.** One person per pitch, replacing the scorekeeper, is the model that can be staffed today and the one every open question above should be resolved against. The per-team model is not abandoned — it is the version that gets both sides named, makes the offence-against-defence cross-tab possible, and is the reason "Two spotters, one from each team" above is written down in the detail it is. But it needs a reliable single-spotter capture before two of them can be reconciled, so it is approached slowly and deliberately second. Where the two designs pull in different directions, the tournament-supplied one wins until it is proven.

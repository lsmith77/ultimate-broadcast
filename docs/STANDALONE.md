...
| **Web server** | Apache with `mod_rewrite` and `AllowOverride All`, or nginx with the rules translated | The `.htaccess` does two jobs: routing the short URLs, and refusing HTTP access to `conf/` except for the three files the stage and the scoreboard poll as static assets. On nginx both become `location` blocks, and **the `conf/` denial is the one that must not be forgotten** — it is what keeps the desk's notes about named players out of a browser. |
...
- **The licence — now settled, though one half remains.** This project is **AGPL-3.0** with a section 7 additional permission for combining with Live! by BULA and UltiOrganizer, and no longer mirrors Live!'s CC BY-NC-ND. Mirroring it was never the safe harbour it looked like: Live! is NC-ND while the UltiOrganizer it runs on is GPL-3.0-or-later, so the precedent was already unsettled, and a Creative Commons licence says nothing about source, linking or patents in the first place. The exception matters most *here*, in the direction this document cares about: standalone links nothing of Live!'s, so no combination occurs and the exception is never engaged — the coupling exists only in hosted mode, in `shared/auth.php`. What remains open is not the licence but the **Terms of Use**, which is a signed contract rather than a licence and may carry its own terms about integrations. That is still a question for the people who signed it.
